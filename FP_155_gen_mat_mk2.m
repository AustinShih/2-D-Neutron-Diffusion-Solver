function [ coef_matrix ] = FP_155_gen_mat_mk2( input_geo, D, sigma_a, sigma_f )
...
for i = 1:N % index through rows
    for j = 1:M % index through columns
            ...
            % matrix
            if i-j == 0
                coef_cell{i,j} =  %populate center matrix
            elseif i-j == -1
                coef_cell{i,j} =  %populate right matrix
            elseif i-j == 1
                coef_cell{i,j} =  %populate left matrix
            else
